...
export default async function parse2(items: any, providerNames: string[], sortMode?: SortMode, filter?: "today") {
	//Return object
	...
	for (let j = 0; j < len; j++) {
		...
		articles.push({
			title: item.title,
			authors: item.authors && item.authors.length > 0 ? item.authors : "",
